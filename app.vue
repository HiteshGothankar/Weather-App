...
<template>
  <div v-if="city" class="min-h-screen relative overflow-hidden">
    ...
    <div
      class="relative z-20 px-6 py-35 sm:px-12 md:px-20 md:py-40 lg:px-48 flex flex-col justify-center h-full"
    >
      <div class="flex flex-col sm:flex-row justify-between items-center gap-6">
        <div class="text-center sm:text-left">
          <h1 class="text-4xl md:text-6xl lg:text-8xl text-white font-semibold">
            {{ city.name }}
          </h1>
          <p class="font-normal text-xl mt-3 text-white">{{ today }}</p>
          <img
            v-if="city.weather && city.weather[0] && city.weather[0].icon"
            :src="`https://openweathermap.org/img/wn/${city.weather[0].icon}@4x.png`"
            alt="Weather Icon"
            class="w-24 h-24 mx-auto sm:mx-0 sm:w-32 sm:h-32"
          />
        </div>
  ...
  <div v-else class="p-10 flex flex-col items-center justify-center h-[100vh]">
    ...
    <button
      class="mt-7 bg-sky-400 px-10 w-40 md:w-50 text-white h-10"
      @click="goBack"
    >
      Go Back
    </button>
